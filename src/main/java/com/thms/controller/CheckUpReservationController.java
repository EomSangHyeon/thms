...
public class CheckUpReservationController {
	...
	@Inject
	private CheckUpRoomService service1;

	@RequestMapping(value = "/register", method = RequestMethod.GET)
	public void ReservationGET(@ModelAttribute("cri") SearchCriteria cri, CheckUpReservationVO vo, Model model)
			throws Exception {
		model.addAttribute("test", service.test(cri));
	}
	...
	@RequestMapping("/register/{chid}")
	@ResponseBody
	public List<CheckUpRoomVO> chlist(@PathVariable("chid") Integer chid) throws Exception {
		return service1.listView(chid);
	}
