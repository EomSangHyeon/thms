...
<body>
	...
	<c:forEach items="${searchmember}" var="search">
		...
		<label><a href="javascript:sendRoom('${search.rmid}')">${search.rmid}</a></label>
		<label>${search.rmnumber}</label>
		<label>${search.rmcurrentcount}</label>
		<label>${search.rmheadcount}</label>
